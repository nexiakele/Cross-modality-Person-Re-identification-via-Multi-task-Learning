...
###############################################################################  
    ...
    def forward(self, x):
        ...
        x3 = fusion + x3
        ##########Re-ID的特征提取 ###################################
        xs1 = self.avgpool(x3).view(x3.size(0), -1)
#        sx26 = sx26.chunk(6, 2)
#        b = sx26[1].size(0)
...
#        ##########多层级特征提取 ###################################
        xxs1  = self.avgpool(fusion).view(fusion.size(0), -1)  
#        ssx26 = fu.chunk(6, 2)
#        b = ssx26[1].size(0)
...
#        xxs6 = ssx26[5].contiguous().view(b,-1)
        ...
        return  xs1,xxs1, maps, ids, (avg1, avg2, avg3, avg4)
...
class embed_net(nn.Module):
    ...
    def forward(self, x1, x2, modal = 0 ):
        ...
        x_0, x_1, maps, ids, avgs  = self.shared(xx)
        ...
        if self.training:
            return (out_0, out_1), (y_0, y_1), maps, ids , avgs
        else:
            x = torch.cat((y_0, y_1), 1)
